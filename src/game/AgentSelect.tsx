...
import { AGENTS, type Agent, type AgentRole } from "@/game/data/agents";
...
import { Lock, ArrowLeft, Loader2, Volume2, MapPin } from "lucide-react";
...
export function AgentSelect({
  ...
  useEffect(() => { prefetchAgentLines(agent); }, [agent]);
    ...
    <div className="fixed inset-0 bg-background text-foreground flex flex-col overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div
          ...
          style={{ background: `radial-gradient(circle at 30% 50%, ${agent.hue}, transparent 60%)` }}
        />
      </div>
      ...
      <header className="relative z-10 border-b border-border bg-sidebar/70 backdrop-blur px-6 h-14 flex items-center justify-between">
        <button onClick={onBack} disabled={locked} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30">
          <ArrowLeft className="w-4 h-4" /> <span className="uppercase tracking-widest text-xs font-bold">Назад</span>
        </button>
        <div className="text-center">
          <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Agent Select</div>
          <div className="text-sm font-black uppercase tracking-widest">
            Карта: <span className="text-[var(--neon)]">{map.name}</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Lock in</div>
          <div className={`text-sm font-black ${locked ? "text-[var(--neon)]" : "text-muted-foreground"}`}>
            {locked ? `${countdown}` : "—"}
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 flex-1 grid grid-cols-12 gap-4 p-4 min-h-0">
        ...
        <aside className="col-span-3 flex flex-col min-h-0">
          <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-2 px-1">Roster · {roster.length}/{AGENTS.length}</div>
          <div className="flex flex-wrap gap-1 mb-2">
            {ROLES.map(r => (
              <button
                ...
                disabled={locked}
                className={`px-2 py-1 text-[10px] uppercase tracking-widest border transition-all ${
                  roleFilter === r
          ...
          {roster.map((a) => {
            ...
            return (
              <button
                key={a.id}
                onClick={() => { if (locked) return; setAgent(a); }}
                disabled={locked}
                className={`w-full flex items-center gap-3 p-2 border transition-all clip-corner text-left ${
                  sel ? "border-[var(--neon)] bg-card/80" : "border-border bg-card/30 hover:bg-card/60"
                }`}
                style={sel ? { boxShadow: `0 0 0 1px ${a.hue}55 inset, 0 0 20px ${a.hue}33` } : undefined}
              >
                <AgentAvatar agent={a} size={44} />
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-bold uppercase tracking-wider truncate">{a.name}</div>
                  <div className="text-[10px] text-muted-foreground uppercase tracking-widest">{a.role}</div>
                </div>
                {sel && <div className="w-1.5 h-8" style={{ background: a.hue, boxShadow: `0 0 10px ${a.hue}` }} />}
              </button>
            );
          })}
          </div>
          ...
          <div className="relative flex-1 border border-border bg-card/30 clip-corner overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-end pointer-events-none">
              ...
              <div className="px-8 pb-8 w-full">
                <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{agent.role}</div>
                <div
        ...
        <aside className="col-span-3 flex flex-col gap-3 min-h-0 overflow-y-auto">
          <div className="border border-border bg-card/50 backdrop-blur p-4 clip-corner">
            <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-1">Identity</div>
            ...
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-3">Personality</div>
            <div className="text-xs" style={{ color: agent.hue }}>{agent.lore.personality}</div>
          </div>

          <div className="border border-border bg-card/50 backdrop-blur p-4 clip-corner">
            <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-2">Abilities</div>
            <AbilityRow k="Q" name={agent.abilities.q} accent={agent.hue} />
            <AbilityRow k="E" name={agent.abilities.e} accent={agent.hue} />
            <AbilityRow k="X" name={agent.abilities.ult} accent={agent.hue} ult />
          </div>

          <div className="border border-border bg-card/50 backdrop-blur p-4 clip-corner">
            <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-2">Voice preview</div>
          ...
          <div className="mt-auto">
            ...
            >
              {locked ? <><Loader2 className="w-4 h-4 animate-spin" /> Загрузка {countdown}</> : <><Lock className="w-4 h-4" /> Lock in</>}
            </button>
          </div>
        </aside>
      </div>
...
function AbilityRow({ k, name, accent, ult }: { k: string; name: string; accent: string; ult?: boolean }) {
  return (
    <div className="flex items-center gap-3 py-1.5">
      <div
        className="w-7 h-7 grid place-items-center text-xs font-black border"
        style={{ borderColor: accent, color: accent, background: ult ? `${accent}22` : "transparent" }}
      >
        {k}
      </div>
      <div className="text-sm">{name}{ult && <span className="ml-1 text-[10px] uppercase tracking-widest text-[var(--neon)]">· Ult</span>}</div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 bg-background text-foreground flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
        <div className="text-[10px] uppercase tracking-[0.5em] text-[var(--neon)] mb-4 animate-pulse">Карта выбрана случайно</div>
        <div className="text-6xl md:text-7xl font-black tracking-tight" style={{ textShadow: "0 0 30px rgba(0,0,0,0.8)" }}>
        ...
        <div className="grid grid-cols-2 gap-3 mt-6 w-full max-w-xl">
          {map.sites.map((s) => (
            <div key={s.key} className="border border-border bg-card/60 backdrop-blur p-3 text-left clip-corner">
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-5">
          {map.designNotes.map((n) => (
            <span key={n} className="text-[10px] uppercase tracking-widest px-2 py-1 border border-border bg-card/40 text-muted-foreground">
              {n}
            </span>
          ))}
        </div>
