...
export function buildAgentModel(agent: Agent): THREE.Group {
  ...
  const matBody = new THREE.MeshStandardMaterial({ color: m.body, roughness: 0.6, metalness: 0.35 });
  const matArmor = new THREE.MeshStandardMaterial({ color: m.armor, roughness: 0.45, metalness: 0.7 });
  const matSkin = new THREE.MeshStandardMaterial({ color: m.head, roughness: 0.75, metalness: 0.05 });
  const matAccent = new THREE.MeshStandardMaterial({
    color: m.visor, roughness: 0.25, metalness: 0.3,
    emissive: m.visor, emissiveIntensity: 0.7,
  });
  const matCloth = new THREE.MeshStandardMaterial({ color: m.body, roughness: 0.9, metalness: 0.05 });

  const mats = { body: matBody, armor: matArmor, skin: matSkin, accent: matAccent, cloth: matCloth };

  switch (agent.silhouette) {
    case "heavy": buildHeavy(g, mats); break;
    case "stealth": buildStealth(g, mats); break;
    case "scout": buildScout(g, mats); break;
    case "mage": buildMage(g, mats); break;
    case "support": buildSupport(g, mats); break;
    case "sniper": buildSniper(g, mats); break;
    case "ninja": buildNinja(g, mats); break;
    case "speedster": buildSpeedster(g, mats); break;
    case "soldier":
    default: buildSoldier(g, mats); break;
  }
...
type Mats = {
  body: THREE.Material; armor: THREE.Material; skin: THREE.Material;
  accent: THREE.Material; cloth: THREE.Material;
};
...
function buildMage(g: THREE.Group, m: Mats) {
  ...
  coat.position.set(0, 1.0, 0); g.add(coat);
  const emblem = new THREE.Mesh(new THREE.CircleGeometry(0.12, 16), m.accent);
  emblem.position.set(0, 1.3, 0.28); g.add(emblem);
  ...
  hood1.position.set(0, 2.55, -0.05); g.add(hood1);
...
function buildSupport(g: THREE.Group, m: Mats) {
  ...
  lamp.position.set(0, 2.34, 0.22); g.add(lamp);
}
...
function buildSniper(g: THREE.Group, m: Mats) {
  ...
  coat.position.set(0, 1.25, 0); g.add(coat);
  ...
  for (const sx of [-0.5, 0.5]) {
    ...
    tuft.position.set(sx, 2.0, 0); g.add(tuft);
  }
  head(g, m.skin, 0.4, 2.32);
  // helmet skullcap
  const skull = new THREE.Mesh(new THREE.SphereGeometry(0.24, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2), m.armor);
  skull.position.y = 2.48; g.add(skull);
  // monocle scope on right eye
  const scope = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.18, 12), m.armor);
  scope.rotation.x = Math.PI / 2;
  scope.position.set(0.12, 2.3, 0.18); g.add(scope);
  const lens = new THREE.Mesh(new THREE.CircleGeometry(0.06, 16), m.accent);
  lens.rotation.y = 0;
  lens.position.set(0.12, 2.3, 0.27); g.add(lens);
}
...
function buildSpeedster(g: THREE.Group, m: Mats) {
  ...
  for (const sx of [-0.18, 0.18]) {
    ...
    fin.position.set(sx, 0.18, 0); g.add(fin);
  }
  torso(g, m.body, 0.72, 0.85, 0.42, 1.42);
  // chest stripe
  const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.6, 0.05), m.accent);
  stripe.position.set(0, 1.45, 0.23); g.add(stripe);
  for (const sx of [-0.46, 0.46]) arm(g, m.body, sx, 0.09, 0.85);
  head(g, m.skin, 0.38, 2.14);
  // aero helmet (sphere top, swept back)
  const aero = new THREE.Mesh(new THREE.SphereGeometry(0.26, 16, 12, 0, Math.PI * 2, 0, Math.PI / 1.6), m.armor);
  aero.position.y = 2.28; g.add(aero);
  // wraparound visor
  const v = new THREE.Mesh(new THREE.SphereGeometry(0.22, 16, 8, 0, Math.PI, Math.PI / 3, Math.PI / 3), m.accent);
  v.rotation.y = Math.PI / 2;
  v.position.set(0, 2.16, 0); g.add(v);
}
