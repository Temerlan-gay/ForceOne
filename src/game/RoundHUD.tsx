...
export function RoundBanner({ round, agent }: { round: RoundState; agent: Agent }) {
  ...
  const phaseColor =
    round.phase === "buy" ? "#ffd166" :
    round.phase === "live" ? "var(--neon)" :
    round.phase === "end" ? "#ff7e5a" : "#a3ff7a";
  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-3 -translate-x-1/2 flex items-center gap-2 animate-fade-in">
      ...
      <div className="bg-card/85 backdrop-blur px-5 py-2 border border-border clip-corner text-center min-w-[180px]">
        ...
        <div className="text-2xl font-black tabular-nums leading-none mt-0.5" style={{ color: phaseColor }}>
          {fmt(remaining)}
        </div>
        <div className="text-[9px] uppercase tracking-widest text-muted-foreground mt-0.5">
          {round.phase === "buy" ? "BUY · Подготовка" : round.phase === "live" ? "LIVE · Бой" : "ROUND END"}
        </div>
      </div>
...
function ScoreChip({ label, value, side, hue }: { label: string; value: number; side: "left" | "right"; hue: string }) {
  return (
    <div
    ...
    >
      ...
      <div className="text-2xl font-black leading-none" style={{ color: hue }}>{value}</div>
    </div>
  );
}
...
export function RoundObjective({ round, agent }: { round: RoundState; agent: Agent }) {
  ...
  return (
    <div className="pointer-events-none absolute left-1/2 top-24 -translate-x-1/2 flex items-center gap-1.5 bg-card/70 backdrop-blur px-3 py-1.5 border border-border text-[10px] uppercase tracking-widest animate-fade-in">
      ...
      <span className="font-bold" style={{ color: agent.hue }}>{round.roundKills}/{need}</span>
  ...
  return (
    <div className="pointer-events-none absolute right-4 top-20 flex flex-col gap-1 items-end">
      {feed.slice(-5).map(e => (
        ...
        >
          <span className="font-bold" style={{ color: e.hue }}>{e.killer}</span>
          <Skull className="w-3 h-3 text-muted-foreground" />
          <span className="font-bold text-foreground/90">{e.victim}</span>
          {e.headshot && <span className="text-[9px] uppercase tracking-widest text-[var(--neon)]">HS</span>}
        </div>
      ))}
    </div>
  );
    ...
    >
      ...
      {bots.map((b, i) => (
        <div
          ...
          style={{
            left: toPx(b.x) - 3, top: toPx(b.z) - 3,
      ...
      <div
        ...
        style={{
          left: toPx(player.x) - 6, top: toPx(player.z) - 6,
      ...
      <div className="absolute bottom-1 left-2 text-[9px] uppercase tracking-widest text-muted-foreground">Mini-map</div>
    </div>
  );
}
...
export function BuyPhaseOverlay({ round, agent }: { round: RoundState; agent: Agent }) {
  ...
  useEffect(() => {
    const id = setInterval(() => setRemaining(Math.max(0, (round.phaseEndAt - performance.now()) / 1000)), 100);
    return () => clearInterval(id);
  }, [round.phaseEndAt]);
  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center animate-fade-in">
      ...
      <div className="relative text-center px-10 py-8 border border-border bg-card/85 clip-corner">
        <div className="flex items-center justify-center gap-2 text-[10px] uppercase tracking-[0.4em]" style={{ color: agent.hue }}>
          <ShoppingCart className="w-3 h-3" /> Buy Phase · Round {round.round}
        </div>
        <div className="text-6xl font-black mt-2 tabular-nums" style={{ color: agent.hue }}>{Math.ceil(remaining)}</div>
        ...
        <div className="mt-4 flex items-center justify-center gap-1 text-[10px] uppercase tracking-widest text-muted-foreground">
          <Timer className="w-3 h-3" /> WASD — двигайся · мышь — обзор · стрельба возобновится в LIVE
        </div>
      </div>
    </div>
  );
...
export function RoundEndOverlay({ round }: { round: RoundState }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center animate-scale-in">
      <div className="px-12 py-6 border-y-2" style={{ borderColor: won ? "var(--neon)" : "#ff4d6d", background: "rgba(10,14,26,0.6)" }}>
        <div className="text-[11px] uppercase tracking-[0.5em] text-muted-foreground text-center">Round {round.round}</div>
        <div className="text-5xl font-black tracking-widest text-center" style={{ color: won ? "var(--neon)" : "#ff4d6d" }}>
...
export function MatchEndOverlay({
  round, agent, onExit,
}: { round: RoundState; agent: Agent; onExit: () => void }) {
  const won = !!round.matchWon;
  const acc = round.totalKills && round.totalDeaths
  ...
  return (
    <div className="pointer-events-auto absolute inset-0 flex items-center justify-center animate-fade-in z-50">
      ...
      <div className="relative max-w-lg w-[90%] border border-border bg-card/90 clip-corner p-8 text-center">
        ...
        <div className="text-6xl font-black mt-2" style={{ color: won ? "var(--neon)" : "#ff4d6d" }}>
          {won ? "VICTORY" : "DEFEAT"}
        </div>
        ...
        <div className="mt-6 border border-border bg-background/40 p-4 clip-corner">
          <div className="flex items-center justify-center gap-2 text-[10px] uppercase tracking-[0.3em]" style={{ color: agent.hue }}>
            <Trophy className="w-3 h-3" /> Round MVP
          </div>
          <div className="text-2xl font-black mt-1" style={{ color: agent.hue }}>{agent.name}</div>
          <div className="text-xs text-muted-foreground mt-1">Best round: {round.bestRoundKills} kills</div>
        </div>
